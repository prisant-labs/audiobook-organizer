...
- Archive folder naming (FD-42, superseding FD-31's "Set Aside"): the physical folder that holds archived items (emptied pack shells, non-preferred-format copies, clutter) is named `Audiobook Archive` on disk, while the product says **Archive** on every surface (the plain-language rule extends to disk artifacts the family can see). Two names on purpose: FD-34 places the folder beside the library at the drive root, where a bare `Archive` could collide with something unrelated and tells a person nothing months later in Explorer, whereas "Audiobook Archive" is repetitive INSIDE an app that is entirely about audiobooks. "quarantine" stays internal-only vocabulary: the `settings.quarantine_root` column, the `quarantine` op kind, and internal type/module names keep it, but it never appears in an on-disk name, a stored rationale sentence, or report copy.
...
- CSP posture. A strict Content-Security-Policy is set in `tauri.conf.json`: `default-src 'self'`, no remote origins, no inline remote script. Combined with the zero-network invariant below, the WebView cannot reach any external host. Landed in v0.4.0 Phase 8 (T-38 release-gate review; `csp` was `null` through P1-P7 while the frontend surfaces were still being built): `"default-src 'self'; connect-src 'self' ipc: http://ipc.localhost; img-src 'self' data:"`. `connect-src` adds the `ipc:`/`http://ipc.localhost` schemes Tauri's own IPC channel needs (otherwise `default-src 'self'` alone blocks every `invoke` call - verified against Tauri's own CSP docs, which show this exact addition). `img-src` adds `data:` for cover art, which arrives over typed IPC as base64 bytes and renders via a `data:` URL (`src/lib/covers.ts`), never a filesystem or `asset:` URL - no `blob:` or `asset:` addition is needed because neither is used. Fonts (bundled woff2, same-origin) and styles (Tailwind's compiled stylesheet, same-origin; React's inline `style` prop sets the DOM `style` IDL property rather than parsing an HTML `style` attribute, so it is not subject to `style-src`) need no separate directive beyond the `default-src 'self'` fallback; the inline theme-boot `<script>` in `index.html` is covered by Tauri's automatic compile-time script hashing (its own CSP docs), not by `'unsafe-inline'`. Verified headed with this CSP applied: see this phase's implementation report for the walkthrough and console/network evidence.

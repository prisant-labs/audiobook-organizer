...
| F-609 (library freshness) NEW | Scan age on the library screen; five scan triggers; a cheap change check on entering the tidy-up flow with a summary before the plan is built | P1 | v0.6.0 | NEW 2026-08-05 per FD-41, from the UI round 2 crit. **Event-triggered, never a filesystem watcher.** Earlier and friendlier surface for the same hazard the apply-time `snapshot-stale` refusal already catches; never a replacement for it. AC-42..AC-46 |
| F-610 (open a folder in the OS file manager) NEW | A backend command that opens Explorer at a path, refusing anything outside the library or set-aside roots; inline affordances plus two sidebar quick links | P1 | v0.6.0 | NEW 2026-08-05, from the UI round 2 crit (jp asked in three places). Cannot be a frontend link: FD-29 gives the web layer no shell access, and granting it would widen the capability allowlist permanently for a convenience. AC-47..AC-50 |
...
| F-1108 (keep-higher-bitrate policy) | P3 | **Deferred out of v0.6.0 on 2026-08-05** (UI round 2 crit). Not deferred for lack of a reader: `lofty` is already a normal dependency and already reads these files for F-907 covers, so bitrate is available. Deferred because **file size is a free proxy**: for the same book a higher-bitrate copy IS a larger file, so `keep-larger` already captures it using a number that cannot be missing or wrong. Also has no well-defined value for a book split across N files. Closes `OQ-1` (per-copy bitrate source) by making it moot |
| F-1109 (folder-change notice on browse) | P3 | **Deferred 2026-08-05.** A count of what changed in the library since the last scan, surfaced while browsing. Distinct from the `snapshot-stale` check, which is a refusal at apply time rather than a notification at browse time. Superseded in priority by the on-entry rescan check (see F-609 in the v0.6.0 spec), which covers the actual risk |
| F-1110 (multi-file book duplicate comparison) | P2 | **Raised 2026-08-05 (UI round 2 crit), not yet scheduled.** All of F-701 to F-704 assumes one book is one file. A book split across twelve mp3s is twelve unrelated files to the detector, so two copies of it are never grouped. Proposed shape: fingerprint the book FOLDER (audio-file count, total bytes, normalised title), then escalate cheap match to structural match (same ordered set of file sizes) to content match (F-702 hashing applied pairwise). Also resolves the m4b-versus-mp3-folder case, which the same-stem F-205 rule does not reach. **Recommended slot: between P2 and P3**, because P3's resolution policies should be written against books rather than files |

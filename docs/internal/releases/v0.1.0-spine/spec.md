...
- Status: implemented; gate walked by Fable 2026-07-04; awaiting jp tag approval (tag cut is human-only per D-10)
...
- Acceptance criteria: AC-1 through AC-24 all satisfied 2026-07-04; per-phase adversarial reviews plus the Fable gate walk below carry the evidence. AC-19 is agent-verified end to end via CDP (screenshot + log evidence in docs/internal/qa/v0.1.0-manual-qa.md); jp spot-check pending, non-blocking.
- Open questions: both resolved (OQ-1: bindings-drift on the Windows runner, see G-5; OQ-2: settings stays the minimal single-row shape, additive columns land with F-803 (app settings) in v0.4.0).
...
- [x] AC-1: The FD-15 OSS-landscape check is recorded (a dated note listing beets audiobook plugins, ABS community organizers, and renamers surveyed, with a one-line "build/adapt/ignore" conclusion) and committed before the first scaffold task. [S: FD-15]

Workspace and core purity:

- [x] AC-2: `cargo build --workspace` succeeds on Windows; the workspace contains `crates/abo-core` and `src-tauri` as members. [S: breakdown Section 4]
- [x] AC-3: The core-purity CI gate passes: `cargo tree -p abo-core -e normal` contains no `tauri` entry (case-insensitive), failing the job if it does. [S: release plan Section 6.1; D-01]
- [x] AC-4: `AppError` and every v0.1.0 IPC payload struct live in `abo-core` and derive `serde::Serialize`, `serde::Deserialize`, and `specta::Type`. [S: reference architecture Section 4]

Database and recovery:

- [x] AC-5: The first migration creates `scans`, `entries`, `jobs`, `settings`, and `activity_records` and applies cleanly against an empty DB. [S: release plan Section 4 gate; breakdown Section 7]
- [x] AC-6: Re-running migration against an already-migrated DB is a no-op (no error, no duplicate objects), proven by a test that opens, migrates, closes, reopens, and migrates again. [S: this brief AC additions]
- [x] AC-7: The pool opens the DB in WAL mode at `%LOCALAPPDATA%\AudiobookOrganizer\abo.db`; a test asserts `PRAGMA journal_mode` returns `wal`. [S: breakdown Section 7; FD-20]
- [x] AC-8: A deliberately corrupted DB file on startup is logged, moved to `corrupt-backups\`, and recreated, with the user notified via `db-corrupt-recovered`; a test drives this path. [S: breakdown Section 7 + Section 8; reference architecture]

F-101 (live tree scanner):

- [x] AC-9: Scanning a small fixture folder yields entries whose count, kinds, sizes, and depths exactly match the fixture's known contents. [S: breakdown F-101]
- [x] AC-10: The scanner opens paths with extended-length (`\\?\`) semantics; a test scans a fixture entry whose full path exceeds 260 characters (generated at runtime, never committed) and records it without error. [S: FD-19]
- [x] AC-11: A permission-denied entry is recorded and the scan continues to completion (no abort); a junction/reparse point is recorded and not followed, and a scan over a fixture containing a junction loop terminates. [S: breakdown F-101; D-09]

F-103 (file typing):

- [x] AC-12: File typing maps each catalogued extension to its class, including `video` for `.mp4`/`.mkv`/`.avi`/`.mov`/`.wmv`/`.m4v` and `comic` for `.cbr`/`.cbz`; a table-driven test covers one example per class and an unknown extension mapping to `other`. [S: breakdown F-103; FD-17]

F-105 (snapshot persistence):

- [x] AC-13: A completed scan persists exactly one `scans` row (with `source = live`, `entry_count`, `total_bytes`, `status = completed`, and timestamps) plus one `entries` row per scanned entry, and the rows are not mutated by a subsequent scan. [S: breakdown F-105]

F-1003 (structured logging):

- [x] AC-14: abo-core emits `tracing` events for scan start and completion; the shell writes a rotating log file via `tauri-plugin-log`; no network egress occurs during a scan (verified by the FD-11 external-host grep over the app plus the absence of any HTTP client in the dependency tree). [S: breakdown F-1003; NFR Privacy]

tauri-specta seam and capability model:

- [x] AC-15: `src/lib/bindings.ts` is generated by tauri-specta and includes the tracer command(s) and the `job:progress`/`job:completed` events; the file is committed and the frontend imports from it, with zero raw `invoke` calls (lint-enforced). [S: reference architecture Section 4]
- [x] AC-16: The bindings-drift CI gate regenerates `bindings.ts` and fails on any diff; its runner placement is recorded per FD-24 (runs on the Windows runner if the specta export links Tauri, else ubuntu), and the chosen placement plus rationale is written into the release-gate evidence. [S: FD-24]
- [x] AC-17: Tauri-family crates and tauri-specta are pinned to exact versions in `Cargo.toml` (no caret/wildcard ranges). [S: D-01; reference architecture]
- [x] AC-18: The main window carries only `event:default` and `core:webview:default` capabilities; no `fs` or `shell` capability is granted to the WebView, verified by inspecting the capability config. [S: FD-29]

Tracer slice:

- [x] AC-19: On Windows, `pnpm tauri dev` launches; clicking the throwaway button runs `scan_start` on a small folder, persists entries, emits `job:completed`, and renders the resulting entries as a JSON dump in the UI. The UI is labeled disposable and is deleted at v0.4.0. [S: release plan Section 4 gate]

CI and hygiene:

- [x] AC-20: The `ci.yml` and `release.yml` workflows from `docs/internal/ci-plan.md` are present and green on the v0.1.0 branch, with concurrency (cancel-in-progress), `permissions: contents: read`, and the LTO profiles per FD-24. The docs-only branch push must not have created a red CI (workflows land live only here). [S: FD-24; release plan Section 6]
- [x] AC-21: The CI matrix is green: lint (fmt, clippy -D warnings, core-purity, typecheck, lint, bindings-drift), test (ubuntu + windows), and build (windows GA + macOS honesty). [S: release plan Section 6.1]
- [x] AC-22: The FD-25 hygiene set is present: `.gitattributes` verified in the branch, `rust-toolchain.toml`, `.nvmrc`, `packageManager` pin, `CHANGELOG.md` with a v0.1.0 entry, `scripts/bump-version.mjs`, and draft `LICENSE`/`CONTRIBUTING.md`/`CODE_OF_CONDUCT.md`/`.github/` templates each marked "pending (D-13)". [S: FD-25]
- [x] AC-23: `.gitignore` ignores `_local/`, `.memsearch/`, and tool caches and is machine-independent (works on a fresh clone with no personal global excludesfile); `docs/internal/` is tracked. [S: D-12; Stream 4 findings 1-2]
- [x] AC-24: `EXECUTION.md` exists at the repo root stating the D-11 governance (trunk-based, short-lived branches, agent self-merge of green PRs while private, CI as code-review substitute). [S: D-11; release plan Section 2]
...
Gate walked by Fable, 2026-07-04. Each phase passed an independent adversarial task review (spec compliance + code quality) with all Critical/Important findings fixed and re-reviewed; the ledger of phase outcomes and recorded debt lives in the P1-P7 reports.

- [x] G-1: All AC-1 through AC-24 satisfied. Evidence: per-phase reviews P1-P7; the AC checkboxes above were flipped only after the Fable gate walk.
- [x] G-2: Tracer slice works end to end on Windows (AC-19). Evidence: docs/internal/qa/v0.1.0-manual-qa.md; agent-driven via CDP inside the live WebView2 (screenshot: 10 entries, correct classes/linkage/depths; abo.log scan records confirmed); jp spot-check pending, non-blocking.
- [x] G-3: CI matrix green: lint, test (ubuntu + windows), build (windows GA artifact confirmed + macOS honesty) (AC-20, AC-21). Evidence: https://github.com/prisant-labs/audiobook-organizer/actions/runs/28703308076 on PR #2.
- [x] G-4: Core-purity gate passes (AC-3). Evidence: ci.yml "Assert abo-core has no tauri dependency" step, green in the run above; verified repeatedly per phase.
- [x] G-5: Bindings-drift gate passes on the WINDOWS runner (AC-16). Placement decision: windows-latest, because the specta export test links Tauri and demonstrably requires the comctl32-v6 manifest (STATUS_ENTRYPOINT_NOT_FOUND 0xc0000139 without it); ubuntu viability unverifiable locally, so FD-24's safe default applies. Rationale also lives as code comments in src-tauri/tests/export_bindings.rs and src-tauri/src/lib.rs. Movable if a future ubuntu trial proves out.
- [x] G-6: Migration applies from empty and existing DB (AC-5, AC-6). Evidence: migrate_from_empty + migrate_idempotent tests, green locally and in CI.
- [x] G-7: FD-15 OSS-landscape check recorded before scaffold work started (AC-1). Evidence: docs/internal/oss-landscape-check.md, dated 2026-07-03, committed a873e38 before any scaffold commit. Verdict: build justified (narrowly).
- [x] G-8: FD-19 extended-length path posture proven (AC-10). Evidence: scan_over_260_char_path test (runtime-generated fixture), green on Windows locally and in CI.
- [x] G-9: FD-25 hygiene set present and FD-29 capability baseline in place (AC-18, AC-22, AC-23, AC-24). Evidence: hygiene files verified in P1 review; capabilities/default.json grants only core:event:default + core:webview:default (the spec's "event:default" shorthand corresponds to Tauri v2's namespaced core:event:default identifier).
- [x] G-10: macOS bundle GREEN in CI (no descope needed; the trigger never fired). Evidence: build-macos job green in the run above.

...
- [ ] **Reference screenshots: three to five, one line each on what to steal.** Drop them in `_local/gui/references/` (currently empty). `D-06` (anti-reference: "looks AI-generated") records what you hate; nothing on file records what you want, so every design decision is currently argued from the negative. This is the only open item nobody but you can do, the only one that raises the ceiling rather than closing a gap, and **the ceiling-raiser for STYLE**, which is the axis nothing else can supply. Note what it does and does not gate: the scale below is a separate, measurable axis that can move without it, and only `shadcn` is strictly sequenced (scale first, then behaviour components).
  ...
  **Where the UI actually stands, so the chain above is legible.** The guardrails are strong and the design language is unratified. Two crit rounds are closed (`FD-39` to `FD-48`); the component gallery is on `main` with 47 specimens rendering the REAL components in both themes; the arbitrary-value ratchet freezes Tailwind sprawl at 285 uses of 71 distinct values and fails in both directions; seven separate sweeps gate user-facing vocabulary. What is missing is the positive direction: what good looks like here has never been written down, because the only artifact that would establish it is the one above.

- [ ] **Ratify a spacing and type scale.** *(Corrected: an earlier draft of this line called it blocked on the screenshots above. It is not, and saying so would have stalled work that can start today. The screenshots raise the ceiling on STYLE; a scale is a different axis and is derivable from the app's own numbers, which is a measurement rather than a taste call. It can be proposed now and reacted to.)* This is the reason the ratchet exists. The ratchet stops the sprawl growing; it does not fix it. The fix is a real scale in `src/styles/tokens.css` under `@theme`, so the utilities the app already uses BECOME the scale. Proposed shape: 4px base, roughly seven spacing steps and seven type steps. I can render a proposal against real components in the gallery for you to react to whenever you want it; it will not be written into `tokens.css` unratified, because that is a tracked normative file and inventing values in it was caught and stopped once already.
- [ ] **`design-system.md`'s type and spacing sections stay wrong until that scale exists.** It states type as RANGES ("h1 26-30px") and says nothing at all about spacing, which is exactly the dimension that drifted. Its vocabulary was 34 instances out of date and was corrected on 2026-08-16; the numbers cannot be corrected the same way, because writing a scale into a normative document is a ratification, not a sweep.
- [ ] **`shadcn/ui` is configured with zero adoption** and no Radix or Base UI dependency installed; `components.json` is complete and `src/components/ui` does not exist. Last in the chain on purpose: the settled order is scale first, THEN shadcn, and shadcn is for BEHAVIOUR only (select, combobox, dialog, popover, tooltip, tabs, checkbox, switch), never domain components.

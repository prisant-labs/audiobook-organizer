...
- [x] **Merge the open pull requests** - DONE on your instruction, **seven times**. First batch of six on 2026-08-15 (`#25`, `#26`, `#27`, `#28`, `#29`, `#30`); second of four, also 2026-08-15 (`#31`, `#32`, `#33`, `#34`); third of two on 2026-08-16 (`#35`, `#36`); fourth of two, also 2026-08-16 (`#38`, `#37`); fifth of five on 2026-08-20 (`#40`, `#41`, `#42`, `#43`, `#39`); sixth of five on 2026-08-21 (`#44`, `#45`, `#47`, `#48`, `#46`); seventh of four on 2026-08-22 (`#49`, `#50`, `#52`, `#51`). **Twenty-eight pull requests, counted from `git log` rather than by adding those batches up.** The previous version of this line said four batches while the paragraph below it said five, and both had dropped the second 2026-08-16 batch entirely, so the running total had been two short since 2026-08-16. `EXECUTION.md` Section 6.2 requires a human decision for any merge to `main`, and each instruction was that decision, recorded here rather than only in a session log. **No authorisation carries forward**: each covered only the pull requests open at that moment. Tags and releases were not touched: those are on the Section 3 human-only list.
...
- [ ] **The Duplicates badge says 406 and the screen lists 300, and reading the record turns this from a choice into a defect** (`AC-18`, `AC-29`, v0.6.0 hardening). The badge counts files bucketed by name and size. The screen counts books, dropping the per-track groups that a duplicated book folder already accounts for. The two reconcile exactly (406, minus 113 groups a book folder already covers, plus 7 book-level groups, is 300), so the diagnosis is certain rather than likely. **The audit that found it offered you two options, and it should not have**: `FD-08` (duplicates canonical unit) says the nav badge counts groups where a group is one book; `AC-18` says every count shown, nav badge included, counts groups; `AC-29` says the badge shows the group count; and the v0.6.0 release gate requires duplicates counted as groups everywhere, naming the badge. Leaving the badge as a file count would mean amending a ratified decision, three acceptance criteria and a release gate. **So the only thing actually open is timing**: fix it before you walk the app, or walk the app first and see the wrong number in place. Detail: [nav badge audit](docs/internal/audits/2026-08-21_nav-badge-count-divergence.md)
...
- [ ] **Decide whether the merged code gets a human read.** **Twenty-eight** pull requests have merged on your instruction plus CI, and CI is not a review. This item said seventeen until 2026-08-23, for the same reason the batch total said twenty-two: a number carried forward by hand rather than counted. **One slice of it has now been read properly**: the five from 2026-08-20 (`#39` to `#43`) got an independent read on 2026-08-21 which closed its own unread list, and it paid for itself, finding the Stop defect that `#48` then fixed and the badge divergence below. Nothing else has had that treatment. So the question is no longer whether a read is worth doing, it is **how much more of the twenty-eight you want read** - none, the duplicates work, or all of it. Say "no review needed" and it stops being an open loop. See the [audits folder](docs/internal/audits/).
...
| v0.6.0 | hardening | in progress | `P0`/`P1`, `P2`, `P2b`, `P3`, `P4`, `P5` **and `P10` all complete**; `P6` unblocked, `P7` to `P9` open | no | `AC-6` walk, `AC-8` walk, the UI walkthrough, the badge count |
| M-1 | campaign | planned | - | - | - |
| v0.9.0 | packaged | planned | - | - | installer, signing decision |

## In flight right now

`main` is at `90e576c` plus this page's own merge commit, CI green, **zero open pull requests and zero local feature branches** (checked with `git rev-parse` at the moment this sentence was written, 2026-08-23, rather than recalled). The page cannot name the commit that merges it, so it names the one it was written against and says so, rather than printing a number that is wrong by one the instant it lands. **Twenty-eight pull requests** have landed across 2026-08-15 to 2026-08-22, in the seven authorised batches listed in the merge item at the top of this page.

import { commands, type PlanGroupView, type PlanOpView, type PlanOpsPage, type PlanReview } from "./bindings";
...
// as a typed `PlanError` (never swallowed), never returned as a sentinel.
...
export async function generatePlan(scanId: number): Promise<PlanReview> {
  ...
  if (result.status === "error") throw new PlanError(formatAppError(result.error));
  return result.data;
}

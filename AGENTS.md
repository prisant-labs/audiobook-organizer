...
> A local-first Windows desktop utility that scans a messy audiobook library, explains what is untidy in plain language, proposes a reorganization as a reviewable dry run, and applies it safely with full undo. Complements Audiobookshelf; never replaces it.
...
- Rollback is "just another plan": undo runs through the same validate, preview, apply pipeline as a forward tidy-up.

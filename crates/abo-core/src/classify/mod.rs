...
//!   - [`overview`] - v0.4.0 Phase 4, F-902: [`overview::build_overview`]
//!     reshapes a classification plus its [`HealthMetrics`] into the
//!     [`crate::ipc::LibraryOverview`] the library home renders (the
//!     `classify_overview` command's payload). Pure, like `engine`/`metrics`/
//!     `multibook`.
...
//!     an activity-log append; `engine`, `metrics`, `multibook`, and
//!     `overview` themselves stay pure, exactly as documented above.
